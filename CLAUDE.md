...
- `web/rest` — `AuthenticateController` (issues JWTs), `AccountResource`, `CareAngelResource` (`/api/care-angels`), `UserResource` (`/api/admin/users`), `PublicUserResource` (`/api/users`), `AuthorityResource`, `GatewayResource` (`/api/gateway/routes`); `web/rest/errors` for RFC 7807 translation; `web/rest/vm` for `LoginVM`, `ManagedUserVM`, `KeyAndPasswordVM`, `RouteVM`.
...
- **`broker` is gone (2026-09-24, `docs/backlog.md` item 61).** It held the JHipster scaffold — `KafkaConsumer`, `KafkaProducer` and the `PatientGatewayKafkaResource` that exposed them at `/api/patient-gateway-kafka/{publish,consume}` — deleted outright, mirroring the api's item 39 cycle 1 deletion of its copy on 2026-09-18. The gateway needs no stream of its own: the membership stream lives in the api and is only routed through here. Two things it did are worth carrying: `kafkaProducer` was a `Supplier<String>` returning the literal `"kakfa_producer"`, which Spring Cloud Stream polls once a second, so this gateway published that string to a topic nothing consumed for as long as it ran; and `/consume` streamed `sse-topic` — to which nothing published — to any authenticated caller with no per-patient filtering, a latent rather than absent hole (`/publish` sent through a binding with no `destination`, so its topic defaulted to the binding name and never reached that fan-out — broker abuse, not a broadcast). The one Kafka function left is `patientEventsConsumer` (`PatientEventMailRouter`), which sends the delegation/erasure mails; `PatientEventFunctionDefinitionTest` holds both YAML files to that name.
- `management` — `SecurityMetersService`, `MailHealthMetrics`, `LoginMetersService` and `RegistrationMetersService`; `aop/logging` — logging aspect.
...
- **Both producers send `subject: {email, login, accountId}`, and `accountId` means the same `User.id` from each — since 2026-09-24 (`docs/backlog.md` item 72), when the architect ended the divergence this bullet used to call deliberate.** It was: this service sent `accountId` (its own `User.id`, which exists from the moment registration returns and is what hc-admin's `Patient.accountId` joins on) while `hc-patient-service` sent `patientId`, an internal profile id that never left that subsystem's vocabulary — two identifiers for the same person on one topic, told apart only by field name. The api now carries `Profile.accountId` in the subject instead, which _is_ this service's `User.id` (its `resolveAccountId()` asks this gateway for it), so a consumer joins either producer's frames on one key. Two residues of the old world remain worth knowing: frames published by the api **before** 2026-09-24 still carry `patientId`, so a replaying consumer must tolerate the old name on old frames; and the api's `accountId` can be **null** on several ordinary paths where this service's is null only by accident — no account in the JWT at onboarding, an account that already holds a profile, or a lost linking race. (Not on a deletion-request `COMPLETED` frame: that read was sequenced after the erasure and item 72's review moved it before, so the completion now carries the id.) ⚠ **"Never" would be wrong about this service too**: `PatientEventPublisher:101` has a branch for a null id that WARNs and **publishes anyway**, on the reasoning that a frame with a key is still attributable — and its own comment names the consequence, which is that hc-admin's `@NotNull Patient.accountId` dead-letters the frame while both stacks look healthy at lag zero. Never by design; a warned-about accident in practice. `PatientEventPublisher.publish` here takes the whole `User` for the same reason the old signature was the defect: with the id as one of four loose arguments it had been hardcoded `null`, and every event this service ever published was id-less.
