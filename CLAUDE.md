...
- **The subject's third component is `accountId` here and `patientId` there, and that is deliberate** (2026-09-18, `docs/backlog.md` item 56). This service sends `subject: {email, login, accountId}` where `accountId` is its own `User.id` — the account, which exists from the moment registration returns and is what hc-admin's `Patient.accountId` joins on. `hc-patient-service` sends `subject: {email, login, patientId}`, an internal profile id that does not exist until onboarding and that its item 54 deletes. **Two identifiers for the same person are on one topic and only the field name tells them apart**, so read `accountId` by name and never "whichever is present". `PatientEventPublisher.publish` takes the whole `User` for the same reason the old signature was the defect: with the id as one of four loose arguments it had been hardcoded `null`, and every event this service ever published was id-less.
